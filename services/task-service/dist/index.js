...
const yoga = createYoga({
    ...
    context: async ({ request }) => {
        ...
        const m = await pool.query(`select role
       from memberships
       where user_id = $1 and org_id = $2
       limit 1`, [auth.sub, auth.orgId]);
        if (m.rowCount === 0) {
        ...
        const userById = new DataLoader(async (ids) => {
            const r = await pool.query(`select id, email, first_name as "firstName", last_name as "lastName"
         from users
         where id = any($1::uuid[])`, [ids]);
            const map = new Map(r.rows.map((u) => [u.id, u]));
            return ids.map((id) => map.get(id) ?? null);
        });
        const teamById = new DataLoader(async (ids) => {
            const r = await pool.query(`select id, name
         from teams
         where org_id = $1 and id = any($2::uuid[])`, [auth.orgId, ids]);
...
app.post("/tasks", async (req, res) => {
    ...
    if (assignedToUserId) {
        const a = await pool.query(`select 1 from memberships where user_id = $1 and org_id = $2`, [assignedToUserId, orgId]);
        if (a.rowCount === 0)
...
app.patch("/tasks/:id", async (req, res) => {
    ...
    const nextTeamId = p.teamId !== undefined ? p.teamId : task.team_id;
    const nextAssignee = p.assignedToUserId !== undefined ? p.assignedToUserId : task.assigned_to_user_id;
    ...
    if (p.assignedToUserId !== undefined && p.assignedToUserId) {
        const a = await pool.query(`select 1 from memberships where user_id = $1 and org_id = $2`, [p.assignedToUserId, orgId]);
        if (a.rowCount === 0)
            return res.status(400).json({ error: "Assignee is not in this org" });
    }
    const fields = [];
    ...
    if (p.teamId !== undefined)
        set("team_id", p.teamId);
    if (p.assignedToUserId !== undefined)
        set("assigned_to_user_id", p.assignedToUserId);
